...
export default {
  ...
  computed: {
    ...
    prediction() {
      let id = this.game.id;
      return this.$store.state.games.predictions.find(
        (elem) => elem.game_id === id
      );
    },
    team_h() {
      if (this.game.type === 0) return this.game.team_h;
      return this.prediction && this.prediction.team_h;
    },
    team_a() {
      if (this.game.type === 0) return this.game.team_a;
      return this.prediction && this.prediction.team_a;
    },
    score_h() {
      return this.prediction && this.prediction.score_h;
    },
    score_a() {
      return this.prediction && this.prediction.score_a;
    },
    winner_team() {
    ...
    finished() {
      return (
        this.prediction &&
        this.prediction.score_h !== null &&
        this.prediction.score_a !== null
      );
    },
    finishedclass() {
    ...
    homeclass() {
      if (this.finished) {
        if (this.prediction.score_h === this.prediction.score_a) {
  ...
  methods: {
    ...
    */
    update_pronotic(game_id, field, value) {
      ...
      this.$store.commit('games/predictionMutation', data);
      this.$store.dispatch('games/updatepredictionAction', data);
    },
    ...
    update_knockout_teams(game) {
      ...
      while (gameCurrent && gameCurrent.type < 4) {        
        ...
        // get prediction for the current game
        const prediction = this.$store.state.games.predictions.find(
          (elem) => elem.game_id === gameCurrent.id
        );
        // if current game is finished
        // be carefull use !=  ,   undefined !== null true, undefined != null false
        if (prediction && prediction.score_h != null && prediction.score_a != null) {
          const field = gameNext.qualification_h === qualification ? 'team_h' : 'team_a';
          const team_id = prediction.score_h > prediction.score_a ? prediction.team_h : prediction.team_a;
